...
function GCam:init(content, ax, ay, xfollow)
	...
	-- Bounds
	self.leftBound = -INF -- -1000000
	self.rightBound = INF -- 1000000
	self.topBound = -INF -- -1000000
	self.bottomBound = INF -- 1000000
	...
	self.__prevPosX = xfollow.pos.x -- 0
	self.__prevPosY = xfollow.pos.y -- 0
...
function GCam:update(dt)
	...
	if self.predictMode then
		...
		if self.predictSmooth > 0 then
			local t = smoothOver(dt, self.predictSmooth, 0.99)
	...
	if -(self.x-dstX)<>(self.x-dstX) > 0.1 or -(self.y-dstY)<>(self.y-dstY) > 0.1 then -- XXX
		self:goto(dstX, dstY)
	end
end
...
function GCam:checkBounds(x,y)
	...
	return clamp(x, self.leftBound+offX, self.rightBound-offX),
		clamp(y, self.topBound+offY, self.bottomBound-offY)
end
...
function GCam:gotoX(x)
	self.x = clamp(x, self.leftBound, self.rightBound)
	self.matrix:setAnchorPosition(self.x, self.y)
	self.viewport:setMatrix(self.matrix)
end
function GCam:gotoY(y)
	self.y = clamp(y, self.topBound, self.bottomBound)
	self.matrix:setAnchorPosition(self.x, self.y)
	self.viewport:setMatrix(self.matrix)
end
